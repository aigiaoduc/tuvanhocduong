...
import { Send, Clock, Sparkles, Users, Mic, MicOff, AlertTriangle, Phone } from 'lucide-react';
        ...
        <div className="p-4 border-t bg-muted/20">
          <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-2">
            ...
            <Input
              ...
              disabled={isLoading || isCrisis}
              placeholder={isCrisis ? "Tính năng chat đã bị khóa để đảm bảo an toàn." : isListening ? "Đang nghe..." : "Nhập câu hỏi của anh/chị..."}
              className="flex-1 h-12 rounded-full px-6 bg-white"
            />
            <Button type="submit" disabled={!input.trim() || isLoading || isCrisis} className="h-12 rounded-full px-6 bg-blue-600 hover:bg-blue-700 text-white">
        ...
        <div className="fixed inset-0 z-[9999] bg-red-950/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative animate-in zoom-in-95 duration-300 border-4 border-red-500">
            ...
            <p className="text-center text-slate-700 mb-6 text-base leading-relaxed">
              Hệ thống nhận thấy anh/chị hoặc con em đang có những suy nghĩ gây hại cho bản thân. Trí tuệ nhân tạo không được phép tư vấn trong các tình huống khẩn cấp về tính mạng.
              <br/><br/>
              Xin hãy gọi ngay cho <strong>Tổng đài Quốc gia Bảo vệ Trẻ em 111</strong> (miễn phí 24/7) hoặc đưa trẻ đến cơ sở y tế/tâm lý gần nhất để được các chuyên gia con người hỗ trợ kịp thời!
            </p>
